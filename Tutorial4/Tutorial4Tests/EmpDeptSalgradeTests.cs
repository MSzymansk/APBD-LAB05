...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldReturnEmployeesFromChicago()
    {
        ...
        List<int> helper = depts.Where(d => d.Loc == "CHICAGO").Select(d => d.DeptNo).ToList();
        List<Emp> result = emps.Where(e => helper.Contains(e.DeptNo)).ToList();
    ...
    public void ShouldCountEmployeesPerDepartment()
    {
        ...
        var result = from e in emps
            group e by e.DeptNo
            into dgroup
            select new { DeptNo = dgroup.Key, Count = dgroup.Count() };
    ...
    public void ShouldCalculateAverageSalaryPerDept()
    {
        ...
        var result = from e in emps
            group e by e.DeptNo
            into dgroup
            select new { DeptNo = dgroup.Key, AvgSal = dgroup.Average(e => e.Sal) };
